...
const PROCESS = [
  { step: "01", title: "Understand", body: "Your business, customers and current presence — before anything else." },
  { step: "02", title: "Strategize", body: "A plan matched to your budget and goals, not a generic package." },
  ...
  { step: "05", title: "Optimize", body: "Real data used to cut what's not working and double down on what is." },
  { step: "06", title: "Scale", body: "More budget, more channels — once the foundation is proven." },
];
          ...
          <div className="section-shell text-center">
            <Reveal className="mx-auto max-w-2xl">
              ...
              <h1 className="mt-6 font-display text-[2.15rem] font-extrabold leading-[1.12] text-white sm:text-5xl lg:text-[3.15rem]">
                Marketing that{" "}
                <span className="text-gradient-marketing">moves your business</span>, not just your feed.
              </h1>
            ...
            <Reveal delay={90} className="mx-auto mt-10 flex flex-wrap items-center justify-center gap-3">
              <Link
                to="/contact"
                  ...
                  <div className="mt-6 grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
                    {group.services.map((service) => {
                      ...
                      return (
                        <article
                          key={service.id}
                          className="group h-full rounded-3xl border border-border bg-card p-6 shadow-soft transition-all duration-300 hover:-translate-y-1.5 hover:border-flame/30 hover:shadow-lift"
                        >
                          ...
                          <h4 className="mt-5 font-display text-base font-bold text-navy">{service.title}</h4>
              ...
              {PROCESS.map((item, i) => (
                <Reveal key={item.step} delay={i * 70}>
                  <div className="h-full rounded-3xl border border-border bg-card p-6 shadow-soft">
                    <span className="font-display text-2xl font-extrabold text-mustard">{item.step}</span>
            ...
            <div className="mt-10 grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-6">
              {WORK_CATEGORIES.map((cat, i) => (
                <Reveal key={cat.title} delay={i * 50}>
                  <div className="flex h-full flex-col items-center gap-3 rounded-2xl border border-border bg-card px-4 py-7 text-center shadow-soft transition-all duration-300 hover:-translate-y-1 hover:shadow-lift">
          ...
          <div className="section-shell">
            <Reveal className="relative isolate overflow-hidden rounded-[2rem] bg-navy px-8 py-14 text-center shadow-lift sm:px-14">
              ...
              <p className="mx-auto mt-3 max-w-lg text-sm leading-relaxed text-white/70 sm:text-base">
                Tell us where your brand is today and where you want it to be — we'll tell you
