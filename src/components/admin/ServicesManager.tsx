...
export function ServicesManager() {
  ...
  async function toggleActive(service: ServiceRecord) {
    setServices((prev) => prev?.map((s) => (s.id === service.id ? { ...s, active: !s.active } : s)) ?? prev);
    try {
  ...
  if (view === "edit") {
    return (
      <div>
        ...
        <h2 className="font-display text-lg font-bold text-navy">{form.id ? "Edit Service" : "New Service"}</h2>
        ...
        <div className="mt-6 grid gap-4 sm:grid-cols-2">
          ...
          <div>
            <label className="mb-1.5 block text-xs font-semibold uppercase tracking-wide text-muted-foreground">Order (lower shows first)</label>
            <input
              type="number"
          ...
          <div className="sm:col-span-2">
            <label className="mb-1.5 block text-xs font-semibold uppercase tracking-wide text-muted-foreground">Category tagline</label>
            <input
          ...
          <div className="sm:col-span-2">
            <label className="mb-1.5 block text-xs font-semibold uppercase tracking-wide text-muted-foreground">One-line description</label>
            <textarea
              value={form.body}
          ...
          <div>
            <label className="mb-1.5 block text-xs font-semibold uppercase tracking-wide text-muted-foreground">Icon</label>
            <select value={form.icon} onChange={(e) => setForm((f) => ({ ...f, icon: e.target.value }))} className={inputClass}>
    ...
    <div>
      ...
      {services.length === 0 ? (
        <p className="mt-8 text-sm text-muted-foreground">
          No services yet — run <code>supabase_migration_003_marketing_services.sql</code> to seed the ones already
          shown on the live page, or add your own.
        </p>
      ) : (
